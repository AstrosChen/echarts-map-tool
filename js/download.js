...
function fetchGeoJson(range, isOnlyOutline, isCompressed, cb) {
    ...
    if(isCentralCity /*&& isOnlyOutline*/ && level == 'city') {
        ...
        keyword = keyword.replace('市市辖区','城区');
    }
    //行政区查询
    district.search(keyword, function(status, result){
        ...
        if (isOnlyOutline) {
            ...
            if (result.districtList[0].districtList) {
                ...
                createOutlineGeojson(result.districtList, isCompressed, cb);//仍然构建全图
            }
        }
    });
}
...
// 不压缩，下载地图文件
function downloadGeoJson(geojson, mapName) {
    ...
    var fileName = [];
    fileName.push(mapName || pinyinUtil.getPinyin(fileStr,'', false, true));
    fileName.push('json');
...
function downloadGeoJs(geojson, mapName) {
    ...
    var str = "(function (root, factory) {"
        ...
        +     "return;"
        + "}"
        + "echarts.registerMap('" + (mapName || pinyinUtil.getPinyin(fileStr,'', false, true)) + "', "
...
function encode(val, prev){
    ...
    // ZigZag
    val = (val << 1) ^ (val >> 15); // -32768 <= val <= 32768
